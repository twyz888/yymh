...
function initImgLazyLoad(errImg) {
	if (errImg == undefined)
		errImg = '/static/aui/theme/image/error-img.png';
...
var appCenter = {
	...
	doGet : function(url, callback) {
		$.ajax({
			...
			success : function(data) {
				if (appCenter.validateJson(data))
					callback(data);
			},
			error : function() {
	...
	doGetBannerData : function(callback) {
		this.doGet('api/banner', function(data) {
			callback(data.result);
		});
	},
	//获取首页数据
	doGetRecommentData : function(callback) {
		this.doGet('api/recommend', function(data) {
			callback(data.result);
		});
	},
	//获取书架数据
	doGetFavoriteData : function(cate, page, callback) {
		this.doGet('api/favorite/' + cate + '-' + page, function(data) {
			callback(data.result);
		});
	},
	//获取排行榜数据
	doGetRankData : function(cate, callback) {
		this.doGet('api/rank/' + cate, function(data) {
			callback(data.result);
		});
	},
	//获取分类数据
	doGetCateData : function(cate, tag, done, order, page, callback) {
		this.doGet('api/cate/' + cate + "-" + tag + "-" + done + "-" + order + "-" + page, function(data) {
			callback(data.result);
		});
	},
	//获取章节列表
	doGetChpaterList : function(aid, order, pageNo, pageSize, callback) {
		this.doGet('../api/chapter_list/' + aid + "-" + order + "-" + pageNo + "-" + pageSize, function(data) {
			callback(data.result);
		});
	},
	//获取章节列表
	doSearch : function(key, pageNo, callback) {
		this.doGet('api/search?keyword=' + key + '&pageNo=' + pageNo, function(data) {
			callback(data.result);
		});
	},
	//修改密码
	doChangePwd : function(oldPwd, newPwd, callback) {
		this.doGet('../api/chang_userpwd?action=oldpwd&oldpwd=' + oldPwd + '&newpwd=' + newPwd, function(data) {
			callback(data.result);
		});
	},
	//短信验证码修改密码
	doSmsCodeChangePwd : function(cellphone, smscode, newPwd, callback) {
		this.doGet('../api/chang_userpwd?action=smscode&mobile=' + cellphone + '&smscode=' + smscode + '&newpwd=' + newPwd, function(data) {
			callback(data.result);
		});
	},
	//根据订单号获取用户
	doFindUserByOrder : function(order, callback) {
		this.doGet('../api/find_user?order_no=' + order, function(data) {
			callback(data.result);
		});
	}
};